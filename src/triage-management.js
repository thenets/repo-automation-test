  ...
  async executeLabelAutomation() {
    ...
    try {
      ...
      if (this.context.eventName === 'pull_request' || this.context.eventName === 'workflow_run') {
        // Extract PR data from the current context
        let prData = null;
        
        if (this.context.eventName === 'pull_request') {
          prData = this.context.payload.pull_request;
        } else if (this.context.eventName === 'workflow_run') {
          // Try to load metadata from artifact first
          const metadata = await this.loadMetadataFromArtifact();
          if (metadata && metadata.type === 'pull_request') {
            prData = metadata;
          } else {
            // Fallback to finding PR by branch
        ...
        const labelResult = await labelAutomation.execute(this.features, prData);
